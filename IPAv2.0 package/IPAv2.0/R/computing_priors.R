...
#' @param unknown.RT The RT number to be assigned to unknown (default NA - No unknown is considered)
...
"compute.Priors" <- function(Hits, dataset, pk = rep(1, nrow(Hits$all.formulas)), ppm, cov_matrix, sd_LogRT, unknown.RT = NA, unknown.ppm = NA, v = T, IT = 120, pr.lim = 1e-05) {
  ...
  if (!is.na(unknown.ppm)) {
    ...
    for (k in 1:M) {
      ...
      delta.unknownRT <- unknown.RT * measurement.LogRT[k]
      pr[k, Nc + 1] <- ((exp((-0.5 * precision[k]) * ((delta.unknownMZ)^2)))) * ((exp((-0.5 * (1/sd_LogRT^2)) * ((delta.unknownRT)^2))))
      if (v) {
        if (k%%IT == 0) {
